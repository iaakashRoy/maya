...
**CEO / CTO / operator field guide · 10 September 2026 · P-001 · apple-launch-continuity**
...
The Apple example asks how to protect an eight-week regional device launch when qualified materials, assembly capacity and transport are constrained at the same time. The current default experiment improves downside service from **47.66% to 69.98%**, but still fails its **95%** service floor. A tool that exposes this failure is more useful than a reassuring dashboard that implies every crisis has been solved.

The default response computes **$3,040,434.07 of mean net loss avoided**, at **$405,014.40 of intervention premiums**. These are conditional model outputs, not achieved Apple savings. They do not validate the much larger **$612M** seeded portfolio narrative. The two numbers have different scopes and different evidence.

A second experiment turns on freight expediting within a 1,800-device weekly ceiling. Downside service rises to **73.09%**, but peak intervention cost reaches **$808,214.40**, above the $700,000 budget. It must be returned for revision, not presented as an approved response.

A third experiment asks what would happen if 30,000 eligible devices were already in opening inventory. It reaches **97.79%** downside service and passes the numerical budget check without expediting. That extra 25,200 devices is **not known to exist**, and its procurement, holding and opportunity cost is not modeled. This is a conditional design requirement to investigate, not a recommended transfer or a proven feasible plan.
...
| Stage | What it represents in the Apple case |
| --- | --- |
| 1. Critical inputs | Battery, magnet, glass, and substrate inputs |
| 2. Advanced wafers | Leading-edge logic and companion silicon |
| 3. Components | Displays, cameras, memory, and power modules |
| 4. Final assembly | New-product introduction and ramp |
| 5. Export gateway | Launch consolidation and customs release |
| 6. Regional hubs | Priority-market inventory allocation |
| 7. Channels | Retail, carrier, and direct fulfillment |
| 8. Customer promise | Launch availability and service protection |
...
| ID / dataset | Intended grain | Declared fixture size / age | Canonical bindings |
| --- | --- | --- | --- |
| P-001-DS-01 · Launch demand contract | SKU × market × week | 684K / 18 sec (modeled) | L0-001, L0-008, L0-016 |
| P-001-DS-02 · Product and component genealogy | Parent part × child part × revision | 1.46M / 3 min (modeled) | L0-028, L0-029, L0-031 |
| P-001-DS-03 · Supplier capacity and qualification | Part × supplier-site × week | 428K / 42 sec (modeled) | L0-044, L0-057, L0-061 |
| P-001-DS-04 · Assembly and yield telemetry | Line × operation × 15 min | 1.82M / 6 sec (modeled) | L0-071, L0-079, L0-088 |
| P-001-DS-05 · Global movement events | Shipment × event time | 412K / 12 sec (modeled) | L0-202, L0-217, L0-229 |
| P-001-DS-06 · Policy and disruption register | Country × policy × effective date | 18.4K / 15 min (modeled) | L0-371, L0-394, L0-445 |

The registry tells a user what data would be needed and at what grain. Counts, freshness and quality percentages are declared fixtures. They are not produced by live S&OP, PLM, QMS, MES, TMS or policy ingestion. Distinguish **dataset ID** `P-001-DS-01` from **table node** `P-001-TABLE-01` and its full-table route key `p-001-table-01`.
...
Open Statistical Studio → Launch demand contract → L0-001 → Descriptive statistics → Run analysis. The first table uses `p-001-table-01`. Export analysis + input rows produces a JSON record of the selected sample, settings and result.

The following figures are computed from the repository's current sample generator, not manually filled into this guide:

| Measure | Full 96-row sample | Filtered: L0-001 ≥ 57 |
| --- | --- | --- |
| Numeric values | 96 | 43 |
| Mean | 56.17 | 65.01 |
| Sample SD | 9.38 | 4.63 |
| P05 | 41.64 | 58.76 |
| Median | 55.13 | 64.3 |
| P95 | 71.7 | 73.04 |
| Missing/nonnumeric | 0 | 0 |

To reproduce the second column of results, open Full table + filters, choose L0-001 and set the minimum to **57**. The table filters its local 96 rows, updates statistics and exports exactly the filtered CSV. It is not a remote SQL query engine. The Statistics page does not inherit the other tab's filters automatically.

### Sample head

| Record | Event time | L0-001 · fixture index |
| --- | --- | --- |
| P-001-R-00001 | 2026-09-01T07:00:00+05:30 | 43.1 |
| P-001-R-00002 | 2026-09-02T08:07:00+05:30 | 73.1 |
| P-001-R-00003 | 2026-09-03T09:14:00+05:30 | 51.01 |
| P-001-R-00004 | 2026-09-04T10:21:00+05:30 | 70.8 |
| P-001-R-00005 | 2026-09-05T11:28:00+05:30 | 53.89 |
| P-001-R-00006 | 2026-09-06T12:35:00+05:30 | 58.87 |
...
| Agent | Declared role | Authority boundary |
| --- | --- | --- |
| Project Orchestrator | Coordinates the governed expert society | Coordinate only; no release authority |
| Evidence Auditor | Challenges sources, transformations, and claims | May block untraceable claims |
| Mineral Scout | Maps mine, refinery, policy, and offtake constraints | Draft scenarios; human approves sources |
| Geopolitical Sentinel | Builds policy and disruption scenarios | No autonomous escalation |
| Demand Anthropologist | Combines behavioral, commercial, and order signals | May draft demand contracts |
| Procurement Strategist | Frames supplier, contract, and award decisions | Draft awards; executive releases |
| Supplier Cartographer | Resolves n-tier identity and dependency | Read and propose graph mappings |
| Manufacturing Planner | Models capacity, yield, schedule, and reliability | Draft plan; plant manager approves |
| Workforce Capacity Analyst | Models skills, rosters, fatigue, and safety | Advisory only |
| Logistics Controller | Plans routes, handoffs, ports, and cargo | Draft movement plans |
| OR Formulator | Translates decisions into auditable mathematical models | Formulate; cannot self-approve |
| Solver Operator | Runs governed solver adapters and records status | Execute approved models only |
...
| Input | Value | Meaning |
| --- | --- | --- |
| Project / scope | apple-launch-continuity | One regional launch cohort; not Apple worldwide production |
| Demand | 10,000 devices/week | Before +12% surge and triangular variation |
| Primary capacity | 11,200 devices/week | Before cut, state factor and yield |
| Alternate capacity | 3,500 devices/week | Available only after qualification; separate yield adjustment |
| Opening available inventory | 4,800 devices | Already eligible finished-equivalent stock; not magnet component sets |
| Opening pipeline | 20,000 devices | Modeled existing dispatches, evenly received over two weeks |
| Additional capacity cut | 32% | Compounds with operating-state capacity loss |
| Demand surge / variation | 12% / ±18% | Elicited stress, not fitted history |
| Yield | 98.4% | Fixed per-unit yield on primary and alternate |
| Alternate available | Week 2 | Default allocation = 100%; no pre-release supply |
| Base lead | 12 days | Opening-pipeline schedule; future supply uses state-specific total lead |
| Expedite ceiling | 1,800 devices/week | Optional one-week shortening, primary first; off by default |
| Expedite unit premium | $28.00 | Charged to expedited units; alternate has an additional 60% sourcing premium |
| Unit margin | $180.00 | Proxy lost contribution; not selling price or audited profit |
| Service floor / budget | 95% / $700,000.00 | P05 service; maximum sampled intervention cost |
| Experiment | 8 weeks / 512 paths / seed 20260909 | Paired random paths; same disturbances for both policies |
...
| Contextual input | Assumed value | What it does / does not mean |
| --- | --- | --- |
| Magnet-ready inventory | 6,200 component sets | Only eligible magnet sets; not interchangeable with finished devices. |
| Priority configuration share | 68 % | Contracted launch mix assumption. |
| Airfreight allocation | 1,800 devices/week | Reserved lane ceiling, not unlimited expedite. |
| Magnet lot quarantine | 11 % | Modeled certificate mismatch removes affected component lots. |
| Assembly overtime ceiling | 8 % | Maximum added hours on qualified lines. |
| Launch freeze | 14 days | Frozen configuration window before release. |
...
| Current state | Total lead days | Capacity factor | Next Normal | Next Constrained | Next Disrupted |
| --- | --- | --- | --- | --- | --- |
| Normal | 12 | 1 | 0.78 | 0.18 | 0.04 |
| Constrained | 21 | 0.78 | 0.2 | 0.58 | 0.22 |
| Disrupted | 35 | 0.48 | 0.08 | 0.32 | 0.6 |
...
Every row uses **supply-flow-monte-carlo@1.1.0**, **512 paths**, **seed 20260909** and an eight-week horizon. Only the changes listed below differ. The archived timestamps are fixed for reproduction; they are not evidence of live activity at that time.

| Experiment | Response P05 | Mean service | Mean net loss avoided | CVaR95 loss | Peak intervention cost | Service / budget |
| --- | --- | --- | --- | --- | --- | --- |
| A · No intervention | 47.66% | 65.98% | $0.00 | $9,023,611.65 | $0.00 | FAIL / PASS |
| B · Current default response | 69.98% | 87.36% | $3,040,434.07 | $5,752,062.41 | $405,014.40 | FAIL / PASS |
| C · Capped expediting | 73.09% | 89.39% | $2,965,798.56 | $5,715,014.74 | $808,214.40 | FAIL / FAIL |
| D · Qualification delay | 62.36% | 79.98% | $1,966,134.15 | $6,876,184.01 | $289,296.00 | FAIL / PASS |
| E · Conditional buffer sensitivity | 97.79% | 99.72% | $785,364.33 | $1,216,062.41 | $405,014.40 | PASS / PASS |
| F · No spending authority | 69.98% | 87.36% | $3,040,434.07 | $5,752,062.41 | $405,014.40 | FAIL / FAIL |

| Experiment / receipt | Change from current UI defaults |
| --- | --- |
| A · No intervention / SIM-5ddcde00 | Alternate used = 0%; expedite off |
| B · Current default response / SIM-eec28899 | 100% alternate from week 2; expedite off |
| C · Capped expediting / SIM-f46feaa4 | B + expedite on; 1,800 devices/week ceiling |
| D · Qualification delay / SIM-86058897 | B, but alternate released in week 4 |
| E · Conditional buffer sensitivity / SIM-414ff846 | B, but opening available inventory = 30,000; unverified +25,200 devices |
| F · No spending authority / SIM-73d1ecf4 | B, but intervention budget = $0 |
...
Restore 100% alternate availability from week 2, with 4,800 devices opening inventory and expediting off. Receipt **SIM-eec28899** reports mean service **87.36%**, downside service **69.98%** and **27.15%** of paths meeting the 95% floor. The tail loss is **$5,752,062.41**. The budget passes, but the service floor fails. Do not route it as a positive operational recommendation.

### C: test the transport lever without unlimited capacity

Turn on expediting and keep the ceiling at 1,800 devices/week. Receipt **SIM-f46feaa4** improves timing, not material availability. Eight weeks of 1,800 fast units at $28 add $403,200 of premiums to the alternate cost. The resulting **$808,214.40** breaks the budget while downside service still misses the floor. This is a visible cost/service trade-off, not an optimizer failure to hide.
...
Set opening available inventory to 30,000 devices, keep the alternate policy and leave expediting off. Receipt **SIM-414ff846** has **97.79%** downside service and passes the modeled budget. Its paired baseline also receives 30,000 opening devices, so **$785,364.33** avoided loss measures the alternate policy's benefit **conditional on that stock**. It is not the benefit of acquiring the extra buffer.
...
| Week | State | Demand | Dispatch | Arrivals | Served | Stock | In transit | Lost |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 1 | Normal | 11,369.34 | 7,494.14 | 10,000 | 11,369.34 | 3,430.66 | 17,494.14 | 0 |
| 2 | Constrained | 9,493.68 | 9,289.43 | 10,000 | 9,493.68 | 3,936.98 | 16,783.58 | 0 |
| 3 | Disrupted | 11,027.32 | 7,041.19 | 10,938.14 | 11,027.32 | 3,847.8 | 12,886.62 | 0 |
| 4 | Disrupted | 11,248.41 | 7,041.19 | 3,444 | 7,291.8 | 0 | 16,483.81 | 3,956.6 |
| 5 | Disrupted | 11,507.32 | 7,041.19 | 9,289.43 | 9,289.43 | 0 | 14,235.57 | 2,217.89 |
| 6 | Disrupted | 11,135.28 | 7,041.19 | 3,444 | 3,444 | 0 | 17,832.76 | 7,691.28 |
| 7 | Disrupted | 11,694.22 | 7,041.19 | 3,444 | 3,444 | 0 | 21,429.95 | 8,250.22 |
| 8 | Constrained | 12,099.95 | 9,289.43 | 7,041.19 | 7,041.19 | 0 | 23,678.19 | 5,058.76 |
...
| ID / parent | Role | Current seeded state | Interpretation |
| --- | --- | --- | --- |
| D0 / root | Outcome | At risk | Launch Continuity · Keep priority launch markets above the governed service floor while reducing single-corridor and single-process dependency. (seeded narrative, not calculated approval) |
| D1 / D0 | Strategic | In review | Allocate constrained components and assembly capacity by market, product, margin, qualification, carbon, and launch promise. (seeded narrative, not calculated approval) |
| D2-A / D1 | Tactical | Candidate | Reserve qualified India assembly headroom, split magnet supply, defer two low-priority configurations, and protect launch-market air capacity. (seeded narrative, not calculated approval) |
| D2-B / D1 | Tactical | Rejected by stress test | Retain baseline · 91.6% P95 launch service (seeded narrative, not calculated approval) |
| D3-A / D2-A | Operational | Validated | Validate No unqualified component source (seeded narrative, not calculated approval) |
| D3-B / D2-A | Operational | Validated | Validate Market service ≥97% for priority launch countries (seeded narrative, not calculated approval) |
| D4 / D3-B | Release | Human gate | Issue governed response after named human approval (seeded narrative, not calculated approval) |
...
| Code | Canonical meaning | Interpretation / examples (not populated production units) |
| --- | --- | --- |
| L0-001 | Demand volume | Quantity requested by a market or customer in a defined period. Examples: units/week; patient visits. |
| L0-008 | Customer service-level requirement | Required probability or proportion of demand fulfilled to commitment. Examples: 98% fill; critical 100%. |
| L0-016 | Forecast error | Difference between forecast and realized demand at a stated aggregation and horizon. Examples: MAPE; forecast bias. |
| L0-028 | Bill-of-material complexity | Count and structural depth of distinct inputs required per output. Examples: 2,000 parts; three-level recipe. |
| L0-029 | Material requirement quantity | Standard quantity of an input required per unit or batch of output. Examples: 2 kg/unit; 4 vials/case. |
| L0-030 | Material availability | Quantity of a specified material accessible at the required place and time. Examples: resin tonnes; donor blood units. |
| L0-031 | Material grade conformance | Degree to which material properties meet the specified grade. Examples: purity; tensile strength. |
| L0-044 | Supplier capacity | Maximum sustainable output a supplier can provide in a stated period. Examples: tonnes/month; API calls/sec. |
| L0-057 | Supplier qualification status | Current approval state of a supplier for a defined scope. Examples: approved; conditional; blocked. |
| L0-061 | Purchase price | Contracted or transacted price paid per purchased unit. Examples: $/kg; hourly rate. |
| L0-071 | Production capacity | Maximum sustainable conforming output of a process in a stated period. Examples: units/hour; procedures/day. |
| L0-079 | Work-in-process quantity | Quantity that has entered but not completed a production process. Examples: open jobs; semi-finished tonnes. |
| L0-088 | Equipment availability | Proportion of required time equipment is capable of operation. Examples: 97% uptime; ready/not ready. |
| L0-155 | On-hand inventory quantity | Physical quantity of an item present at a location at a point in time. Examples: units; tonnes; beds. |
| L0-202 | Freight-volume requirement | Quantity of cargo requiring movement on a lane in a stated period. Examples: tonnes/week; parcels/day. |
| L0-217 | Transport-capacity availability | Quantity of compatible transport capacity offered for a lane, mode, and time window. Examples: truck slots; tonnes/day. |
| L0-229 | Port-to-port schedule reliability | Proportion of vessel services arriving within the defined schedule tolerance. Examples: within 24 h; blank-sailing effect. |
| L0-275 | Delivery-promise accuracy | Agreement between promised and realistically achievable delivery date or window. Examples: available-to-promise accuracy; ETA error. |
| L0-299 | Landed unit cost | Total assigned cost per unit at a destination, including purchase, logistics, duty, and handling. Examples: delivered $/kg; import cost. |
| L0-371 | Scope 1 emissions | Direct greenhouse-gas emissions from owned or controlled sources in a stated period. Examples: onsite fuel; refrigerant leaks. |
| L0-394 | Drought severity | Magnitude and duration of water deficit affecting a source, basin, or operation. Examples: drought index; reservoir level. |
| L0-445 | Risk-tolerance threshold | Maximum approved exposure or performance deviation before mandatory action. Examples: days at risk; VaR limit. |
| L1-001 | Demand sensing and forecasting | Converts consumption, order, market, and historical signals into governed demand projections. Examples: SKU forecast; patient-demand outlook. |
| L1-006 | Strategic sourcing | Selects supply markets and allocates demand among sources under cost, capacity, risk, and policy constraints. Examples: dual source; local source. |
| L1-008 | Procurement and contracting | Converts requirements into authorized orders and contracts with controlled price, terms, quantity, and timing. Examples: sourcing event; PO release. |
| L1-010 | Production planning and control | Translates demand and material availability into feasible capacity, sequence, batch, and release plans. Examples: master schedule; dispatch list. |
| L1-019 | Inventory policy and replenishment planning | Sets buffer, reorder, allocation, ageing, and replenishment parameters by item and echelon. Examples: safety stock; reorder policy. |
| L1-032 | Multimodal transport planning and control | Selects, books, monitors, and replans mode, route, capacity, consolidation, and intermodal handoffs. Examples: sea-air; rail-road; route control. |
| L1-041 | Supply-chain cost, pricing, and profitability management | Measures and manages purchase, conversion, inventory, logistics, duty, service, and margin economics. Examples: landed cost; cost-to-serve. |
| L1-054 | Regulatory, tax, trade, and market-access management | Maintains compliance with product, labor, environmental, customs, sanctions, tax, privacy, IP, and procurement rules. Examples: export control; product licence. |
| L2-001 | Demand and market volatility | Emergent instability in demand level, mix, timing, channel, price response, and customer behavior. Examples: promotion spike; rapid mix shift. |
| L2-002 | Supply scarcity and concentration | Composite constraint created by limited, inflexible, geographically concentrated, or fragile upstream supply. Examples: sole-source API; mineral shortage. |
| L2-003 | End-to-end capacity pressure | Network-wide tension arising when demand approaches or exceeds available supplier, production, labor, storage, handling, transport, or node capacity. Examples: peak-season overload; surge demand. |
| L2-006 | End-to-end service performance | Emergent customer outcome created by availability, promise quality, fulfilment completeness, timeliness, condition, and response. Examples: OTIF; critical-patient service. |
| L2-008 | Cost inflation and margin pressure | System-wide pressure on unit economics created by input, labor, energy, logistics, tax, financing, inefficiency, and pricing movements. Examples: freight inflation; margin squeeze. |
| L2-013 | Geopolitics, conflict, and sanctions | Cross-border force produced by state rivalry, armed conflict, sanctions, embargoes, border action, and political stability. Examples: war-zone closure; sanctions regime. |
| L2-027 | Supply-chain resilience and continuity | Emergent ability to anticipate, absorb, adapt to, recover from, and learn from disruptions while maintaining critical outcomes. Examples: dual-site recovery; adaptive allocation. |
| L2-034 | Environmental resource scarcity | Composite constraint created by limited water, land, energy, biomass, minerals, ecosystem services, and compliant waste capacity. Examples: basin water stress; critical-mineral constraint. |
...
| Code / method | Purpose | Evidence that production use requires |
| --- | --- | --- |
| M-02 · Time-series, causal, and hierarchical forecasting | Create reconciled point and probabilistic forecasts at SKU, family, region, and network levels. | Rolling-origin backtest; Bias / MASE / WAPE; Pinball loss; Interval coverage. Limit: Structural breaks and unavailable future drivers can dominate historical accuracy. |
| M-03 · Intermittent-demand forecasting | Estimate occurrence and size for sparse service parts and low-frequency materials. | Backtest service and cost; Compare with naive sparse baseline; Calibration by intermittency class. Limit: Aggregate error metrics can hide costly stockout behavior for rare critical items. |
| M-04 · Bayesian inference and state estimation | Update latent demand, disruption, failure, and supplier-state beliefs as evidence arrives. | Prior sensitivity; Posterior calibration; Posterior predictive replay. Limit: Weak priors or correlated evidence can create unjustified certainty. |
| M-05 · Linear programming and minimum-cost network flow | Allocate divisible flow through a network at lowest landed cost or highest served value. | Primal and dual feasibility; Flow conservation; Baseline improvement. Limit: Continuous relaxations cannot represent indivisible setups, lots, or yes/no choices without extension. |
| M-06 · Mixed-integer linear programming | Optimize networks with discrete facilities, suppliers, lanes, setups, assignments, and batches. | Integrality and feasibility; Bound / gap integrity; Small-instance enumeration; Replay fingerprint. Limit: Runtime can grow sharply with weak formulations, symmetry, or excessive scenarios. |
| M-13 · Single- and multi-echelon inventory optimization | Place buffers and replenishment policies across coupled echelons to meet service at minimum cost. | Backtest cycle service and fill rate separately; Monte Carlo policy test; EOQ / ROP baseline comparison. Limit: Cycle service and fill rate are not interchangeable; service assumptions and dependency among echelons can materially change recommended stock. |
| M-16 · Monte Carlo simulation | Propagate correlated uncertainty through a selected plan to estimate outcome distributions and tail risk. | Seed reproducibility; Monte Carlo half-width; Dependence tests; Out-of-sample stress. Limit: A precise simulation of a misspecified distribution remains misleading. |
| M-17 · Discrete-event simulation | Represent queues, resources, calendars, routing, failures, and recovery in time-ordered operations. | Conceptual validation; Code and conservation tests; Warm-up / run length; Calibration; Operator face validation. Limit: Detailed simulation can be expensive to build and may encode unreviewed operating rules. |
| M-20 · Two-stage and multistage stochastic programming | Choose here-and-now commitments and scenario-dependent recourse without using future information. | Probability reconciliation; Non-anticipativity test; Held-out scenarios; Scenario-reduction sensitivity. Limit: Scenario explosion and poor joint distributions can make a detailed model falsely confident. |
| M-22 · Robust and distributionally robust optimization | Retain feasibility or limit loss across bounded parameter sets or probability ambiguity sets. | Budget / radius sensitivity; Historical extremes; Held-out regimes; Set coverage. Limit: Over-wide sets create conservatism; narrow sets miss real compound extremes. |
| M-23 · CVaR and risk-averse optimization | Trade expected performance against the mean loss in the worst governed tail. | Alpha / lambda sensitivity; Tail stability; Named-stress replay; Out-of-sample tail loss. Limit: Historical tails may be too sparse to identify compound risk without stress design. |
| M-24 · Multiobjective and Pareto optimization | Expose defensible trade-offs among service, cost, resilience, emissions, cash, safety, and social outcomes. | Unit normalization; Weight / epsilon sensitivity; Hard-floor compliance; Decision-owner review. Limit: A frontier informs judgment but does not decide governance preferences by itself. |
| M-26 · Simulation-optimization, rolling horizon, and digital twins | Continuously estimate state, generate candidates, simulate, select, execute, and replan after drift. | Shadow mode; Closed-loop replay; Synchronization test; Fallback drill. Limit: A stale twin can automate yesterday’s reality; state and policy drift require active monitoring. |
...
| Source file | Audit responsibility |
| --- | --- |
| app/PlatformShell.tsx | Inspected for current behavior; SHA-256 is retained in the companion evidence pack |
| app/ProjectWorkspace.tsx | Data, Playground, policy drafts and project decision lineage |
| app/workspace-model.ts | Inspected for current behavior; SHA-256 is retained in the companion evidence pack |
| app/case-study-model.ts | Inspected for current behavior; SHA-256 is retained in the companion evidence pack |
| app/os-case-evidence.ts | Inspected for current behavior; SHA-256 is retained in the companion evidence pack |
| app/ProjectSupplyChainExplorer.tsx | Inspected for current behavior; SHA-256 is retained in the companion evidence pack |
| app/GlobalKnowledgeGraph.tsx | Inspected for current behavior; SHA-256 is retained in the companion evidence pack |
| app/statistical-model.ts | Generated sample identities and nonphysical fixture indices |
| app/statistical-analysis.ts | Inspected for current behavior; SHA-256 is retained in the companion evidence pack |
| app/StatisticalStudio.tsx | Inspected for current behavior; SHA-256 is retained in the companion evidence pack |
| app/table/TableExplorerClient.tsx | Inspected for current behavior; SHA-256 is retained in the companion evidence pack |
| app/simulation-model.ts | Executable Monte Carlo, flow balance, caps, versioning and restoration |
| app/SimulationStudio.tsx | Inspected for current behavior; SHA-256 is retained in the companion evidence pack |
| app/decision-evidence-model.ts | Local evidence attachment, stale checks, review conditions and expiry |
| app/DecisionToolEvidence.tsx | Inspected for current behavior; SHA-256 is retained in the companion evidence pack |
| app/OptimizationWorkbench.tsx | Inspected for current behavior; SHA-256 is retained in the companion evidence pack |
| app/ProjectAppStudios.tsx | Inspected for current behavior; SHA-256 is retained in the companion evidence pack |
| app/project-activity-model.ts | Inspected for current behavior; SHA-256 is retained in the companion evidence pack |
| worker/index.ts | Web-serving adapter, not an agent/backend runtime |

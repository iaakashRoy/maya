...
type OutcomeHandler = (title: string, detail: string, artifact?: string) => void;
...
const defaultSessionFor = (project: WorkspaceProject): ProjectSessionState => ({
  ...
  traceIndex: -1,
  runState: "Ready",
  selectedAgentId: expertAgents[0].id,
  ...
  assignedExperts: humanExperts.map((item) => item.id),
});
const projectSessionCache: Record<string, ProjectSessionState> = {};
const requireWorkspaceProject = (id?: string) => {
  const project = id ? workspaceProjects.find((item) => item.id === id) : workspaceProjects[0];
...
const traceStepsFor = (project: WorkspaceProject, prompt: string, steering: readonly string[]): readonly TraceStep[] => [
  ...
  { state: "Reading", agent: "Evidence Auditor", title: "Read project-scoped evidence", detail: `6 dataset views · ${project.counts.claims} claim metadata · no cross-client traversal`, nodes: ["src", "supplier"] },
  { state: "Traversing", agent: "Domain Cartographer", title: "Trace the outcome path", detail: `${project.client} source → ${project.variablePack.l0[0]} → operating node → ${project.metrics[1].label}`, nodes: ["supplier", "material", "plant", "order"] },
  { state: "Formulating", agent: "OR Formulator", title: "Build the decision-model draft", detail: `${project.methodCodes.slice(0, 4).join(" + ")} · project variables · 12 canonical constraint families${steering.length ? ` · steering: ${steering.join("; ")}` : " · no steering overrides"}`, nodes: ["supplier", "material", "calc"] },
...
export default function ProjectWorkspace({ onOpenApp, onOpenCase, onOutcome = () => undefined, initialProjectId, initialTab = "overview", initialApp = null, onProjectChange, onTabChange, onStudioChange }: ProjectWorkspaceProps) {
  const firstProject = requireWorkspaceProject(initialProjectId);
  ...
  const [studioApp, setStudioApp] = useState<ProjectAppId | null>(initialApp && !isExistingApp(initialApp) ? initialApp : null);
  ...
  const project = requireWorkspaceProject(projectId);
  const graphNodes = graphNodesFor(project);
  const traceSteps = traceStepsFor(project, activePrompt, steeringInstructions);
  const selectedAgent = [...expertAgents, ...createdAgents].find((item) => item.id === selectedAgentId) ?? expertAgents[0];
  ...
  const filteredProjects = useMemo(() => {
    ...
    return q ? workspaceProjects.filter((item) => `${item.sector} ${item.client} ${item.name} ${item.problem}`.toLowerCase().includes(q)) : workspaceProjects;
  }, [portfolioQuery]);
  ...
  const selectProject = useCallback((next: WorkspaceProject, push = true) => {
    projectSessionsRef.current[projectIdRef.current] = {
      ...
      draftAgentName, createdAgents, selectedExpertId, assignedExperts,
    };
    ...
    setSelectedAgentId(restored?.selectedAgentId ?? expertAgents[0].id);
    ...
    setAssignedExperts(restored?.assignedExperts ?? humanExperts.map((item) => item.id));
    onProjectChange?.(next);
    onTabChange?.("overview");
    if (push) updateUrl(next, "overview");
  }, [activePrompt, assignedExperts, chatMessages, chatText, createdAgents, draftAgentName, mountedApps, onProjectChange, onStudioChange, onTabChange, runState, selectedAgentId, selectedDecision, selectedExpertId, selectedGraphNode, sessionDatasets, steeringInstructions, traceIndex, updateUrl, uploadName, uploadStage]);
  ...
  }, [activePrompt, assignedExperts, chatMessages, chatText, createdAgents, draftAgentName, mountedApps, projectId, runState, selectedAgentId, selectedDecision, selectedExpertId, selectedGraphNode, sessionDatasets, steeringInstructions, traceIndex, uploadName, uploadStage]);

  useEffect(() => {
    const restore = () => {
      const url = new URL(window.location.href);
      const restoredProject = workspaceProjects.find((item) => item.id === url.searchParams.get("project"));
      const restoredTab = workspaceTabs.find((item) => item.id === url.searchParams.get("projectTab"))?.id;
      const restoredApp = projectApps.find((item) => item.id === url.searchParams.get("projectApp"))?.id ?? null;
      ...
      setStudioApp(restoredApp && !isExistingApp(restoredApp) ? restoredApp : null);
    };
    restore();
  ...
  return (
    <div className="project-os" data-page-heading tabIndex={-1}>
      ...
      <section className="project-stage">
        <MobileProjectSwitcher project={project} onSelect={selectProject} />
        ...
        {tab === "overview" && <OverviewPanel project={project} mounted={mountedApps} onTab={changeTab} onOpenCase={onOpenCase} onEvidence={openEvidence} onOpenApp={openApp} />}
        ...
        {tab === "data" && <DataPanel project={project} uploadStage={uploadStage} uploadName={uploadName} sessionDatasets={sessionDatasets} onFile={setUploadFile} onUseSample={() => { setUploadName(`${project.code}_Project_Sample.csv`); setUploadStage("Staged"); }} onAdvance={nextUploadStage} onEvidence={openEvidence} onOutcome={onOutcome} />}
      ...
      {builderOpen && <AgentBuilder project={project} step={builderStep} name={draftAgentName} onName={setDraftAgentName} onStep={setBuilderStep} onClose={() => setBuilderOpen(false)} onPublish={(draft) => { const sequence = createdAgents.length + 1; const id = `draft-${project.code.toLowerCase()}-${String(sequence).padStart(2, "0")}`; setCreatedAgents((current) => [...current, { id, name: draftAgentName, role: draft.specialty, level: "Apprentice", years: 0, evaluatedRuns: 0, approvedRuns: 0, calibration: 0, overrideRate: 0, failureRate: 0, skills: draft.skillFile ? [...draft.skills, `selected-file:${draft.skillFile}`] : draft.skills, mcps: draft.connections.filter((item) => !item.includes("tool")).map((item) => `${item} · requested`), tools: draft.connections.filter((item) => item.includes("tool")).map((item) => `${item} · draft only`), authority: `Draft manifest inside ${project.client} / ${project.name}; no skill parsing, MCP connection, solver execution, or release`, state: "Draft" }]); setSelectedAgentId(id); setBuilderOpen(false); onOutcome("Agent draft saved", `${draftAgentName} was added as browser-session manifest ${sequence} for ${project.client} / ${project.name}; no Skills.md content was read, MCP connected, evaluation executed, tool created, or agent deployed.`, `AGENT-${project.code}-DRAFT-${String(sequence).padStart(2, "0")}`); }} />}
    </div>
  );
}

function MobileProjectSwitcher({ project, onSelect }: { project: WorkspaceProject; onSelect: (project: WorkspaceProject) => void }) {
  const sectorIds = Array.from(new Set(workspaceProjects.map((item) => item.sectorId)));
  const sectorProjects = workspaceProjects.filter((item) => item.sectorId === project.sectorId);
  ...
  return <fieldset className="mobile-project-switcher"><legend>ACTIVE WORKSPACE PATH</legend><label><span>Sector</span><select value={project.sectorId} onChange={(event) => onSelect(workspaceProjects.find((item) => item.sectorId === event.target.value) ?? project)}>{sectorIds.map((id) => { const item = workspaceProjects.find((candidate) => candidate.sectorId === id)!; return <option value={id} key={id}>{item.sector}</option>; })}</select></label><label><span>Client</span><select value={project.clientId} onChange={(event) => onSelect(workspaceProjects.find((item) => item.sectorId === project.sectorId && item.clientId === event.target.value) ?? project)}>{clientIds.map((id) => { const item = sectorProjects.find((candidate) => candidate.clientId === id)!; return <option value={id} key={id}>{item.client}</option>; })}</select></label><label><span>Project</span><select value={project.id} onChange={(event) => onSelect(workspaceProjects.find((item) => item.id === event.target.value) ?? project)}>{clientProjects.map((item) => <option value={item.id} key={item.id}>{item.name} · {item.code}</option>)}</select></label></fieldset>;
}
...
function ProjectTree({ selected, projects, query, onQuery, onSelect }: { selected: WorkspaceProject; projects: readonly WorkspaceProject[]; query: string; onQuery: (value: string) => void; onSelect: (project: WorkspaceProject) => void }) {
  ...
  return <aside className="project-tree" aria-label="Sector, client, and project tree"><header><span>WORKSPACES</span><b>Client delivery</b><small>{portfolioTotals.sectors} sectors · {portfolioTotals.clients} clients · {portfolioTotals.projects} projects</small></header><label className="tree-search"><span>⌕</span><span className="sr-only">Find sector, client, or project</span><input value={query} onChange={(event) => onQuery(event.target.value)} placeholder="Find sector, client, project" /></label>{queryActive && <button className="tree-clear-search" type="button" onClick={() => onQuery("")}>Clear search to collapse branches</button>}<div className="workspace-tree-list" role="tree">{sectors.map((sector, sectorIndex) => { const sectorOpen = queryActive || expandedSectors.includes(sector.id); return <section className="sector-tree-node" key={sector.id}><button data-action-id={`workspace.sector.${sector.id}`} className={sector.id === selected.sectorId ? "active-context" : ""} type="button" aria-expanded={sectorOpen} disabled={queryActive} onClick={() => setExpandedSectors((current) => toggle(current, sector.id))}><i>{String(sectorIndex+1).padStart(2,"0")}</i><span><small>SECTOR TOWER</small><b>{sector.label}</b></span><strong>{sectorOpen ? "−" : "+"}</strong></button>{sectorOpen && <div className="client-tree-list">{sector.clients.map((client) => { const clientOpen = queryActive || expandedClients.includes(client.id); return <div className="client-tree-node" key={client.id}><button data-action-id={`workspace.client.${client.id}`} className={client.id === selected.clientId ? "active-context" : ""} type="button" aria-expanded={clientOpen} disabled={queryActive} onClick={() => setExpandedClients((current) => toggle(current, client.id))}><span>⌞</span><b>{client.label}</b><i>{clientOpen ? "−" : "+"}</i></button>{clientOpen && <div className="project-tree-list">{client.projects.map((project) => <div className={`tree-branch ${selected.id === project.id ? "open" : ""}`} key={project.id}><button data-action-id={`workspace.select.${project.id}`} type="button" className={selected.id === project.id ? "active" : ""} aria-current={selected.id === project.id ? "page" : undefined} onClick={() => onSelect(project)}><i>└</i><span><small>PROJECT · {project.code}</small><b>{project.name}</b><em>{project.problem}</em></span><strong className={`project-health health-${project.health}`} /></button>{selected.id === project.id && <div className="tree-active-children"><span>Decisions · {project.counts.decisions}</span><span>Data · {project.counts.observations}</span><span>Apps · {project.counts.apps}</span><span>Agents · {project.counts.agents}</span></div>}</div>)}</div>}</div>; })}</div>}</section>; })}</div><div className="tree-portfolio-note"><b>UI ISOLATION DEMO</b><span>Raw facts are presented inside one project at a time. Production isolation still requires backend authorization, row policies, encryption, and audit controls.</span></div></aside>;
}
...
function OverviewPanel({ project, mounted, onTab, onOpenCase, onEvidence, onOpenApp }: { project: WorkspaceProject; mounted: readonly ProjectAppId[]; onTab: (tab: WorkspaceTabId) => void; onOpenCase: () => void; onEvidence: (ref: string | EvidenceReceipt) => void; onOpenApp: (id: ProjectAppId) => void }) {
  ...
  return <div className="project-overview"><section className="project-thesis"><p>PROJECT OUTCOME</p><h2>{project.outcome}</h2><div><button data-action-id="workspace.open-governed-decision" type="button" onClick={onOpenCase}>Open governed decision →</button><button data-action-id="workspace.ask-expert-society" type="button" onClick={() => onTab("agents")}>Ask the expert society</button></div></section><div className="project-metric-grid">{project.metrics.map((metric) => <button data-action-id={`evidence.open.${metric.evidenceRef}`} className={`metric-${metric.tone}`} type="button" key={metric.label} onClick={() => onEvidence(metric.evidenceRef)}><span>{metric.label}</span><strong>{metric.value}</strong><small>{metric.detail}</small><em>◇ Trace {metric.evidenceRef}</em></button>)}</div><div className="project-home-grid"><section><header><p>DECISION TREE</p><button data-action-id="workspace.open-decisions" type="button" onClick={() => onTab("decisions")}>Open all</button></header><ol>{projectDecisions.slice(0,4).map((item) => <li key={item.id}><span>{item.id}</span><div><b>{item.title}</b><small>{item.level} · {item.owner} · {item.value}</small></div></li>)}</ol></section><section><header><p>MOUNTED APP GRAPH</p><button data-action-id="workspace.configure-apps" type="button" onClick={() => onTab("apps")}>Configure</button></header><div className="mini-app-graph">{mounted.slice(0,5).map((id, index) => { const app = projectApps.find((item) => item.id === id)!; return <span className="mini-app-entry" key={id}>{index > 0 && <i>{index % 2 ? "feeds" : "challenges"}</i>}<button data-action-id={`workspace.open-app.${id}`} type="button" onClick={() => onOpenApp(id)} style={{color:app.accent}}>{app.name}</button></span>; })}</div></section></div><section className="portfolio-footprint"><header><div><p>PROJECT KNOWLEDGE FOOTPRINT</p><h2>One governed memory, many specialist views</h2></div><button data-action-id="workspace.open-project-data" type="button" onClick={() => onTab("data")}>Open Project Data →</button></header><div>{[[project.counts.entities,"entities"],[project.counts.relationships,"relationships"],[project.counts.observations,"L0 observations"],[project.counts.documents,"documents"],[project.counts.events,"events"],[project.counts.claims,"evidence claims"]].map((item) => { const receipt = fixtureEvidenceFor(project, { id: `EV-FOOTPRINT-${item[1]}`, claim: `Project knowledge footprint: ${item[1]}`, displayedValue: item[0], source: "Project knowledge-footprint deterministic fixture", formula: `Declared fixture count of ${item[1]} inside the selected project boundary`, inputs: [project.code, project.client, item[1]], variableId: "Project registry metadata", grain: `Project × ${item[1]}` }); return <button data-action-id={`workspace.trace-footprint.${item[1]}`} type="button" key={item[1]} onClick={() => onEvidence(receipt)}><b>{item[0]}</b><span>{item[1]}</span><em>◇</em></button>; })}</div><footer><span>Taxonomy contract · 481 L0 · 60 L1 · 35 L2</span><b>Method registry · M-01 through M-30</b></footer></section></div>;
}
...
function AppsPanel({ project, mounted, onOpen, onEvidence }: { project: WorkspaceProject; mounted: readonly ProjectAppId[]; onOpen: (id: ProjectAppId) => void; onEvidence: (target: string | EvidenceReceipt) => void }) {
  ...
  return <div className="apps-os">
    <header className="section-hero"><div><p>PROJECT APP GRAPH</p><h2>Apps are specialist views over one project memory</h2><span>Mount each app separately. Inputs, outputs, agents, variable bindings, and evidence stay attached to this project.</span></div><button data-action-id="apps.trace-mount-manifest" type="button" onClick={() => onEvidence(mountReceipt)}>Trace mount manifest ◇</button></header>
...
function DataPanel({ project, uploadStage, uploadName, sessionDatasets, onFile, onUseSample, onAdvance, onEvidence, onOutcome }: { project: WorkspaceProject; uploadStage: string; uploadName: string; sessionDatasets: readonly { id: string; name: string; rows: string; state: string }[]; onFile: (event: ChangeEvent<HTMLInputElement>) => void; onUseSample: () => void; onAdvance: () => void; onEvidence: (target: string | EvidenceReceipt) => void; onOutcome: OutcomeHandler }) {
  ...
  return <div className="data-vault">
    <header className="section-hero"><div><p>PROJECT DATA VAULT</p><h2>Preview a governed ingestion contract</h2><span>The browser captures a filename only. The schema, mapping, quality, and merge stages below are explicit synthetic fixtures—not results from reading or transmitting the file.</span></div><span className="truth-chip">METADATA-ONLY · NO BACKEND</span></header>
    ...
    <section className="dataset-register"><header><div><p>PROJECT DATA PRODUCTS</p><h2>{projectDatasetViews.length + sessionDatasets.length} governed dataset views</h2></div><button data-action-id="data.connection-request" type="button" onClick={() => onOutcome("Connection request drafted", `A least-privilege source-connection request was created for ${project.client}; no credential or MCP server was provisioned.`, `CONNECTION-${project.code}-04`)}>Request connection →</button></header><div className="table-scroll"><table><thead><tr><th>Dataset</th><th>Source / grain</th><th>Records</th><th>Freshness</th><th>Quality</th><th>Taxonomy bindings</th><th>State</th><th /></tr></thead><tbody>
    ...
    <section className="future-adapters"><header><p>SECURE EDGE → CLOUD ROADMAP</p><h2>Data capture where work happens</h2></header>{[["Port & cargo mobile","GS1 Digital Link / QR scan","Signed handoff event","Future adapter"],["Robot & machine edge","OPC-UA / MQTT gateway","Telemetry + work completion","Future adapter"],["Carrier & vessel","Licensed AIS / carrier API","Position, ETA, cargo state","Disconnected"],["Flight telemetry","Licensed ADS-B provider","Aircraft and air-cargo state","Disconnected"],["Client source agent","Read-only CDC / API","Project-scoped evidence","Concept contract"]].map((adapter) => <article key={adapter[0]}><b>{adapter[0]}</b><span>{adapter[1]}</span><small>{adapter[2]}</small><em>{adapter[3]}</em><button data-action-id={`data.adapter.${adapter[0]}`} type="button" onClick={() => onOutcome("Adapter requirements opened", `${adapter[0]} requires licensing, project authorization, credentials, retention policy, and production integration.`, `ADAPTER-${adapter[0].toUpperCase().replaceAll(" ", "-")}`)}>Inspect</button></article>)}</section>
  </div>;
}
...
function GraphPanel({ project, nodes, traceSteps, selectedNode, onSelect, selected, traceIndex, onEvidence, onSteer }: { project: WorkspaceProject; nodes: ReturnType<typeof graphNodesFor>; traceSteps: readonly TraceStep[]; selectedNode: string; onSelect: (id: string) => void; selected: ReturnType<typeof graphNodesFor>[number]; traceIndex: number; onEvidence: (target: string | EvidenceReceipt) => void; onSteer: (label: string) => void }) {
  ...
  return <div className="graph-os"><header className="section-hero"><div><p>KNOWLEDGE GRAPH + TRACE PLAYBACK</p><h2>Inspect the deterministic path, then steer its replay</h2><span>This concept exposes a prompt-bound fixture path—not hidden model reasoning. Every node shown here is simulated; production must visibly distinguish observed, corroborated, inferred, simulated, and proposed facts.</span></div><span className="truth-chip">7 NODES SHOWN · {project.counts.entities} SYNTHETIC PROJECT ENTITIES</span></header><div className="graph-layout"><section className="project-knowledge-canvas"><div className="graph-grid-lines" />{projectGraphEdges.map((edge,index) => <span className={`graph-edge ge${index+1}`} key={`${edge[0]}-${edge[1]}`}><i />{edge[2]}</span>)}{nodes.map((node) => <button data-action-id={`graph.select.${node.id}`} style={{left:`${node.x}%`,top:`${node.y}%`}} className={`project-graph-node kind-${node.kind.toLowerCase()} ${selectedNode === node.id ? "selected" : ""} ${activeNodes.includes(node.id) ? "tracing" : ""}`} type="button" key={node.id} onClick={() => onSelect(node.id)}><small>{node.kind}</small><b>{node.label}</b><span>{node.detail}</span></button>)}</section><aside className="graph-sidecar"><p>SELECTED NODE</p><h2>{selected.label}</h2><span>{selected.kind} · {selected.detail}</span><button data-action-id={`graph.evidence.${selected.id}`} type="button" onClick={() => onEvidence(selectedReceipt)}><small>EVIDENCE RECEIPT</small><b>{selectedReceipt.id}</b><em>Open exact fixture manifest →</em></button><section><p>VISUAL STEERING</p>{["Pin as assumption", "Exclude this source", "Make a hard constraint", "Assign specialist", "Request alternative path"].map((action) => <button data-action-id={`graph.steer.${action}`} type="button" key={action} onClick={() => onSteer(action)}>{action}<span>+</span></button>)}</section></aside></div><section className="trace-playback"><header><p>VISIBLE AGENT TRAVERSAL</p><span>{traceIndex >= 0 ? `Step ${traceIndex+1} of ${traceSteps.length}` : "Open Agent Society and start a run"}</span></header>{traceSteps.map((step,index) => <article className={index < traceIndex ? "done" : index === traceIndex ? "active" : ""} key={step.title}><span>{String(index+1).padStart(2,"0")}</span><div><small>{step.state} · {step.agent}</small><b>{step.title}</b></div></article>)}</section></div>;
}

function AgentPanel({ project, traceSteps, selectedAgent, selectedAgentId, onSelectAgent, messages, chatText, onChatText, onSubmit, traceIndex, runState, onAdvance, onCancel, onSteer, onEvidence, onOpenBuilder, createdAgents }: { project: WorkspaceProject; traceSteps: readonly TraceStep[]; selectedAgent: ExpertAgent; selectedAgentId: string; onSelectAgent: (id: string) => void; messages: readonly { role: "user" | "agent" | "system"; text: string }[]; chatText: string; onChatText: (value: string) => void; onSubmit: (event: FormEvent) => void; traceIndex: number; runState: string; onAdvance: () => void; onCancel: () => void; onSteer: (label: string) => void; onEvidence: (target: string | EvidenceReceipt) => void; onOpenBuilder: () => void; createdAgents: readonly ExpertAgent[] }) {
  const roster = [...expertAgents, ...createdAgents];
...
function TeamPanel({ project, selected, selectedId, assigned, onSelect, onAssign, onEvidence }: { project: WorkspaceProject; selected: typeof humanExperts[number]; selectedId: string; assigned: readonly string[]; onSelect: (id: string) => void; onAssign: (id: string) => void; onEvidence: (target: string | EvidenceReceipt) => void }) {
  ...
  return <div className="team-os"><header className="section-hero"><div><p>HUMAN EXPERTS IN THE LOOP · SYNTHETIC PROFILES</p><h2>Accountability belongs to governed human roles</h2><span>These illustrative people, experience levels, workloads, and availability states demonstrate the future workflow; no HR, calendar, or staffing system is connected.</span></div><span className="truth-chip">{assigned.length} ASSIGNED · {humanExperts.length} FIXTURE PROFILES</span></header><div className="team-layout"><section className="expert-grid">{humanExperts.map((expert) => <button data-action-id={`team.select.${expert.id}`} className={selectedId === expert.id ? "active" : ""} type="button" key={expert.id} onClick={() => onSelect(expert.id)}><span>{expert.initials}</span><div><small>{expert.role}</small><h3>{expert.name}</h3><p>{expert.specialties.join(" · ")}</p></div><em>{expert.years}y fixture</em></button>)}</section><aside className="expert-inspector"><span>{selected.initials}</span><p>{selected.role} · synthetic</p><h2>{selected.name}</h2><small>{selected.availability} · fixture state</small><dl><div><dt>Domain-profile experience</dt><dd>{selected.years} years</dd></div><div><dt>Fixture workload</dt><dd>{selected.activeWork} items</dd></div><div><dt>Proposed decision right</dt><dd>{selected.decisionRight}</dd></div></dl><section><p>SPECIALTIES</p>{selected.specialties.map((item) => <b key={item}>{item}</b>)}</section><button className="expert-trace-action" data-action-id={`team.trace.${selected.id}`} type="button" onClick={() => onEvidence(profileReceipt)}>Trace profile fixture ◇</button><button data-action-id={`team.assign.${selected.id}`} type="button" onClick={() => onAssign(selected.id)}>{assigned.includes(selected.id) ? "Remove from project" : "Assign to project"}</button></aside></div></div>;
}
...
function GovernancePanel({ project, onOutcome, onEvidence }: { project: WorkspaceProject; onOutcome: OutcomeHandler; onEvidence: (target: string | EvidenceReceipt) => void }) {
  ...
  return <div className="governance-os"><header className="section-hero"><div><p>PROJECT GOVERNANCE</p><h2>Trust is part of the operating model</h2><span>{project.classification} · {project.dataResidency} · owner {project.owner}</span></div><button data-action-id="governance.export-manifest" type="button" onClick={() => onOutcome("Governance summary receipt recorded", `The current ${project.code} policy labels and concept boundaries were recorded in the browser-session ledger; no manifest file was generated.`, `GOV-MANIFEST-${project.code}`)}>Record manifest summary →</button></header><section className="governance-controls">{controls.map((control,index) => <button data-action-id={`governance.control.${index+1}`} type="button" key={control[0]} onClick={() => index === 2 ? onEvidence(coverageReceipt) : onOutcome("Control description opened", `${control[0]} · ${control[1]} · concept state ${control[2]}.`, `CONTROL-${project.code}-${String(index+1).padStart(2,"0")}`)}><span>{String(index+1).padStart(2,"0")}</span><div><b>{control[0]}</b><p>{control[1]}</p></div><em>{control[2]}</em></button>)}</section><div className="governance-grid"><section><p>AGENT CREATION POLICY</p><h3>Identity → Skills.md → MCP/tools → boundaries → evaluation → human review → shadow</h3><ol><li>Self-created tools begin quarantined.</li><li>Permission expansion always requires a human.</li><li>Experience derives from evaluated outcomes—not self-description.</li><li>Promotion requires security and model-risk approval.</li></ol></section><section><p>FUTURE GEOSPATIAL POLICY</p><h3>Basemap is not agent memory</h3><ol><li>Google 3D Tiles require billing, key security, attribution, and policy compliance.</li><li>OpenSky / ADS-B / AIS feeds require valid commercial rights.</li><li>Licensed imagery and telemetry are never silently cached or learned from.</li><li>Current map and tracks are deterministic demo fixtures.</li></ol></section><section><p>LEARNING LEDGER</p><h3>Project retention without unsafe autonomy</h3><ol><li>Record policy version, action, reward definition, outcome, expert rating, and counterfactual.</li><li>Challenge before promotion; preserve rollback.</li><li>Cross-client learning requires consent and approved aggregation.</li><li>No reinforcement-learning claim until a policy is actually updated and evaluated.</li></ol></section></div></div>;
}
...
function AgentBuilder({ project, step, name, onName, onStep, onClose, onPublish }: { project: WorkspaceProject; step: number; name: string; onName: (name: string) => void; onStep: (step: number) => void; onClose: () => void; onPublish: (draft: AgentDraft) => void }) {
  ...
  return <div className="builder-overlay">
    ...
    <section className="agent-builder" role="dialog" aria-modal="true" aria-label="Create a project agent draft">
      <header><div><p>AGENT FOUNDRY · FRONT-END CONCEPT</p><h2>Draft a governed project-specialist manifest</h2></div><button data-action-id="agent-builder.close" type="button" aria-label="Close agent builder" onClick={onClose}>×</button></header>

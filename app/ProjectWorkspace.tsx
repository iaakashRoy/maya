...
export default function ProjectWorkspace({ onOpenApp, onOpenCase, onOutcome = () => undefined, initialProjectId, initialTab = "overview", initialApp = null, initialSessionId = null, initialRunId = null, onTabChange, onStudioChange, onSessionChange, onRunChange, projects = workspaceProjects, collaborators = workspaceCollaborators, memberships = projectMemberships, activeCollaboratorId = signedInCollaboratorId, activityState, dispatchActivity, onMountedAppsChange, onAgentRosterChange, onProjectSetupChange }: ProjectWorkspaceProps) {
  ...
  if (studioApp) return <div className="project-os studio-mode" data-page-heading tabIndex={-1}><section className="project-stage"><ProjectAppStudio appId={studioApp} project={project} onEvidence={openEvidence} onOutcome={onOutcome} />{studioApp !== "simulation" && <AppRunHistory key={focusedAppRunId ?? `studio-${studioApp}`} project={project} runs={projectAppRuns.filter((run) => run.appId === studioApp)} activityState={activityState} dispatchActivity={dispatchActivity} onOpen={openApp} onOpenSession={openWorkSession} onEvidence={openEvidence} onOutcome={onOutcome} canRun={evaluateProjectAccess(project.id, activeCollaboratorId, "agents.run", memberships).allowed && dataContractReady} runBlockedReason={!dataContractReady ? "Complete Data setup before starting an application run." : undefined} focusedRunId={focusedAppRunId} initialAppId={studioApp} onRunChange={onRunChange} />}</section>{evidence && <EvidenceDrawer receipt={evidence} onClose={() => setEvidence(null)} onOutcome={onOutcome} />}</div>;

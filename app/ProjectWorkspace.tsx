...
import DecisionToolEvidence from "./DecisionToolEvidence";
import { downloadJson } from "./SimulationStudio";
import { decisionEvidenceKey, restoreDecisionArtifacts } from "./decision-evidence-model";
...
export default function ProjectWorkspace({ onOpenApp, onOpenCase, onOutcome = () => undefined, initialProjectId, initialTab = "overview", initialApp = null, initialSessionId = null, initialRunId = null, onTabChange, onStudioChange, onSessionChange, onRunChange, projects = workspaceProjects, collaborators = workspaceCollaborators, memberships = projectMemberships, activeCollaboratorId = signedInCollaboratorId, activityState, dispatchActivity, onMountedAppsChange, onAgentRosterChange, onProjectSetupChange }: ProjectWorkspaceProps) {
  ...
  if (studioApp) return <div className="project-os studio-mode" data-page-heading tabIndex={-1}><section className="project-stage"><ProjectAppStudio appId={studioApp} project={project} onEvidence={openEvidence} onOutcome={onOutcome} onOpenOptimizer={() => openApp("optimizer")} />{studioApp !== "simulation" && studioApp !== "consumer-insights" && <AppRunHistory key={focusedAppRunId ?? `studio-${studioApp}`} project={project} runs={projectAppRuns.filter((run) => run.appId === studioApp)} activityState={activityState} dispatchActivity={dispatchActivity} onOpen={openApp} onOpenSession={openWorkSession} onEvidence={openEvidence} onOutcome={onOutcome} canRun={evaluateProjectAccess(project.id, activeCollaboratorId, "agents.run", memberships).allowed && dataContractReady} runBlockedReason={!dataContractReady ? "Complete Data setup before starting an application run." : undefined} focusedRunId={focusedAppRunId} initialAppId={studioApp} onRunChange={onRunChange} />}</section>{evidence && <EvidenceDrawer receipt={evidence} onClose={() => setEvidence(null)} onOutcome={onOutcome} />}</div>;
...
function DecisionPanel({ project, selected, onSelect, onEvidence, onOpenCase, onOutcome, draftReady, onCreateDraft, canReview, reviewer }: { project: WorkspaceProject; selected: string; onSelect: (id: string) => void; onEvidence: (ref: string) => void; onOpenCase: () => void; onOutcome: OutcomeHandler; canReview: boolean; reviewer: string; draftReady: boolean; onCreateDraft: () => void }) {
  useEffect(() => {
    const requested = window.location.hash.replace("#decision-", "");
    if (!decisionsFor(project).some((decision) => decision.id === requested)) return;
    const timer = window.setTimeout(() => onSelect(requested), 0);
    return () => window.clearTimeout(timer);
  }, [project, onSelect]);
  const projectDecisions = decisionsFor(project);
  const exportDecisions = (nodes: ReturnType<typeof decisionsFor>, kind: string) => {
    let evidence = null;
    try { evidence = restoreDecisionArtifacts(localStorage.getItem(decisionEvidenceKey(project.id)), project.id).filter((artifact) => nodes.some((node) => node.id === artifact.decisionId)); } catch { /* Export lineage even if local evidence is unavailable. */ }
    downloadJson({ projectId: project.id, projectCode: project.code, exportedAt: new Date().toISOString(), decisions: nodes, calculatedEvidence: evidence, variables: project.variablePack, methods: project.methodCodes, boundary: "Scenario lineage and local calculated evidence. Not a production approval or release." }, `${project.code}-${kind}.json`);
  };
  ...
  return <div className="decision-os">
    <section className="decision-tree-panel decision-lineage-panel">
      <header><div><p>DECISION LINEAGE</p><h2>Branch, challenge, and merge</h2><span>A Git-style history of decision versions and approvals. Evidence lineage remains in Data.</span></div><button data-action-id="decisions.open-case" type="button" onClick={onOpenCase}>Open decision</button></header>
      <div className="decision-lineage-toolbar" role="group" aria-label="Decision lineage view">
        ...
        <button data-action-id="decisions.record-snapshot" type="button" onClick={() => exportDecisions(projectDecisions, "lineage")}><span>◇</span>Export snapshot</button>
      </div>
      {lineageView === "history" ? <div className="decision-commit-list" aria-label="Decision branch history">
        ...
        <div className="decision-merge-gate"><span>✓</span><div><small>MERGE GATE · NOT RELEASED</small><b>{project.outcome}</b><p>{project.counts.experts} named expert roles · {project.methodCodes.length} method references · human approval required</p></div><button data-action-id="decisions.merge-review" type="button" onClick={() => { const panel = document.getElementById("decision-calculated-evidence"); panel?.scrollIntoView({ block: "center", behavior: "smooth" }); panel?.focus(); }}>Review selected result</button></div>
  ...
  return <div className={`agent-os ${sessionsOpen ? "" : "sessions-collapsed"} ${inspectorOpen ? "" : "inspector-collapsed"}`}>
    ...
    <section className="agent-session" aria-label={`${project.name} agent terminal`}>
      <div ref={transcriptRef} className="agent-messages" role="log" aria-live="polite" aria-label="Scrollable agent transcript and activity">
        ...
        </article>) : <div className="agent-session-zero"><b>Start the first project session</b><p>Send a brief below. tanjnx will create a stable session ID and visible scope receipt. Conversation stays available even while project data is being configured.</p></div>}
...
function ProjectMembershipsPanel({ project, members, onEvidence }: { project: WorkspaceProject; members: readonly ProjectMemberView[]; onEvidence: (target: string | EvidenceReceipt) => void }) {
  return <section className="project-memberships">
    <header><div><p>PROJECT MEMBERS</p><h2>Client and tanjnx access</h2><span>Memberships are scoped to {project.client} / {project.name}.</span></div><span>{members.length} MEMBERS</span></header>
...
function GovernancePanel({ project, onOutcome, onEvidence }: { project: WorkspaceProject; onOutcome: OutcomeHandler; onEvidence: (target: string | EvidenceReceipt) => void }) {
  const controls = [
    ...
    { id: "human-gate", icon: "◎", tone: "approval", name: "Human release approval", detail: "Require a named client or tanjnx decision right before release.", defaultOn: true, critical: true },

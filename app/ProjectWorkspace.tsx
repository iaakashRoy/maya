  ...
  return (
    <div className={`project-os ${tab === "agents" ? `playground-app-mode ${playgroundFullscreen ? "playground-fullscreen" : ""}` : ""}`.trim()} data-project-tab={tab} data-page-heading tabIndex={-1}>
      <section className="project-stage">
        {tab === "agents" ? <header className="playground-app-header"><AppGlyph appId="playground" label="Playground" /><div><p>PROJECT APPLICATION · {project.code}</p><h1>Playground</h1><span>{project.client} / {project.name} · live project agent console</span></div><aside><span><i />Terminal ready</span><button data-action-id="agents.toggle-fullscreen" type="button" aria-pressed={playgroundFullscreen} onClick={() => setPlaygroundFullscreen((current) => !current)}><b aria-hidden="true">{playgroundFullscreen ? "↙" : "↗"}</b>{playgroundFullscreen ? "Exit full screen" : "Full screen"}</button></aside></header> : <><header className="project-commandbar"><div><p>{project.code} · Project workspace</p><h1>{project.name}</h1><span>{project.problem}</span></div><aside><small>PROJECT STATE</small><b><i className={`project-tone-${project.health}`} />{project.stage}</b><span>{project.classification} · {project.dataResidency}</span></aside></header>
...
function AgentPanel({ project, dataReady, traceSteps, selectedAgent, selectedAgentId, onSelectAgent, sessions, selectedSession, messages, activities, appRuns, sessionsOpen, inspectorOpen, onToggleSessions, onToggleInspector, onSelectSession, onContinueSession, onOpenRun, chatText, onChatText, onSubmit, traceIndex, runState, onAdvance, onCancel, onSteer, onEvidence, onOpenBuilder, agents }: AgentPanelProps) {
  ...
  const quickPrompts = [
    `Summarize the latest work on ${project.name}.`,
    "Challenge the current assumptions and list missing evidence.",
    "Formulate the next decision and stop at human review.",
  ];
  ...
  return <div className={`agent-os ${sessionsOpen ? "" : "sessions-collapsed"} ${inspectorOpen ? "" : "inspector-collapsed"}`}>
    ...
    <section className="agent-session">
      <header className="playground-runner-bar">
        <div className="agent-console-identity"><span className="playground-ready"><i />Ready to chat<small>{activityLabel} · {dataReady ? "Project tools and evidence are available" : "Data setup needed for tools"}</small></span><div className="agent-console-title"><p>{selectedSession ? `${selectedSession.id} · ${selectedSession.entryPoint} entry` : "NEW PROJECT SESSION"}</p><h2>{selectedSession?.title ?? `${project.name} work session`}</h2></div></div>
        <div className="agent-console-actions"><span className={`run-state run-${runState.toLowerCase()}`}>{runState === "Running" ? "Trace running" : selectedSession?.status ?? runState}</span><label><span>Run as</span><select aria-label="Select Playground agent" value={selectedAgentId} onChange={(event) => onSelectAgent(event.target.value)}>{agents.map((agent) => <option value={agent.id} key={agent.id}>{agent.name}</option>)}</select></label>{runState === "Running" ? <button data-action-id="agents.stop-run" className="agent-stop-run" type="button" onClick={onCancel}>Stop</button> : <button data-action-id="agents.open-builder" type="button" onClick={onOpenBuilder}>New agent</button>}</div>
      </header>
      ...
      <div ref={transcriptRef} className="agent-messages" role="log" aria-live="polite" aria-label="Scrollable agent transcript and activity">
        ...
        <section className={`agent-runtime-feed runtime-${runState.toLowerCase()}`} aria-label="Agent thinking and tool activity" aria-live="polite">
          <header><div><i /><span><b>Agent activity</b><small>{activityLabel}</small></span></div><code>{selectedSession?.id ?? "no-session"}</code><button data-action-id="agents.activity.toggle" type="button" aria-expanded={activityOpen} onClick={() => setActivityOpen((current) => !current)}>{activityOpen ? "Hide" : "Show"}</button></header>
          {activityOpen && (runState === "Ready" ? <div className="agent-runtime-idle"><code>$</code><span>Waiting for an instruction. Use <b>@agent</b>, <b>/commands</b>, or attach project files below.</span><i /></div> : <div className="agent-runtime-steps">{traceSteps.map((step, index) => {
            ...
            return <button data-action-id={`agents.trace.${index + 1}`} className={`runtime-step state-${state}`} type="button" key={step.title} onClick={() => onEvidence(receipt)}><span><i />{String(index + 1).padStart(2, "0")}</span><div><small>{step.state}</small><b>{step.title}</b><p>{step.detail}</p></div><code>{step.agent}</code><em>{state === "done" ? "Done" : state === "active" ? "Working" : state === "review" ? "Review" : state === "stopped" ? "Stopped" : "Queued"}</em></button>;
          })}</div>)}
        </section>

        {result && <section className="agent-final-result"><span>FINAL RESULT · HUMAN REVIEW REQUIRED</span><h3>{result.headline}</h3><p>{result.recommendation}</p><div>{result.metrics.map((metric) => <dl key={metric.label}><dt>{metric.label}</dt><dd>{metric.value}</dd></dl>)}</div><footer>{result.evidenceRefs.map((ref) => <button data-action-id={`agents.result.evidence.${ref}`} type="button" key={ref} onClick={() => onEvidence(ref)}>{ref}</button>)}</footer><small>{result.reviewGate} · {result.claimBoundary}</small></section>}
      </div>

      <div className="agent-steering-bar">{["Pause + pin assumption", "Add service constraint", "Reject source", "Compare robust model"].map((action) => <button data-action-id={`agents.steer.${action}`} type="button" disabled={!selectedSession || !dataReady} key={action} onClick={() => onSteer(action)}>{action}</button>)}</div>

      <form className="agent-prompt" onSubmit={submitPrompt}>
        <div className="agent-composer-toolbar"><button data-action-id="agents.attach-file" type="button" aria-keyshortcuts="Control+U Meta+U" onClick={() => attachmentRef.current?.click()}><span aria-hidden="true">＋</span> File</button><button data-action-id="agents.open-mentions" className={mentionsOpen ? "active" : ""} type="button" aria-expanded={mentionsOpen} aria-controls="agent-mention-menu" onClick={() => setMentionsOpen((current) => !current)}><span aria-hidden="true">@</span> Agents</button><button data-action-id="agents.insert-command" type="button" onClick={insertCommand}><span aria-hidden="true">/</span> Commands</button><input ref={attachmentRef} className="agent-file-input" type="file" multiple accept=".csv,.xlsx,.json,.pdf,.parquet,.docx,.md,.txt" onChange={attachFiles} /><span>Project-scoped browser session</span></div>

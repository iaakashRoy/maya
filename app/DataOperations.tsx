    ...
    <div className="data-workspace agent-workspace">
      ...
      <div className="data-grid">
        <section className="panel agent-detail">
          <header className="panel-header"><div><p className="kicker">SELECTED AGENT</p><h2>{selectedAgent.name}</h2></div><span className="model-status">POLICY SIGNED · {selectedStatus.toUpperCase()}</span></header>
          ...
          <dl><div><dt>Runtime status</dt><dd>{selectedStatus}</dd></div><div><dt>Current freshness</dt><dd>{selectedStatus === "paused" ? "Paused" : selectedAgent.freshness}</dd></div><div><dt>Execution boundary</dt><dd>{selectedAgent.boundary}</dd></div><div><dt>Credential mode</dt><dd>Customer-managed identity</dd></div><div><dt>Write access</dt><dd>Off · approval required</dd></div><div><dt>Raw retention</dt><dd>Customer policy</dd></div><div><dt>Cross-tenant training</dt><dd>Disabled</dd></div><div><dt>Evidence lineage</dt><dd>100% retained</dd></div></dl>
          ...
          <div className="decision-box" aria-live="polite">
            {detailMode === "policy" ? <><span>POLICY VIEW · SYNTHETIC</span><b>Read-only observation inside {selectedAgent.boundary}</b><p>Customer-managed identity · source write access off · lineage retained · cross-tenant training disabled.</p></>
              : detailMode === "runs" ? <><span>RECENT RUNS · SYNTHETIC FIXTURE</span><b>{lastTestAgentId === selectedAgent.id ? "Dry run completed just now" : `Last modeled run ${selectedAgent.freshness}`}</b><p>{selectedStatus === "running" ? `${selectedAgent.records} modeled records · ${selectedAgent.quality}% quality · no source read or write.` : `Runtime ${selectedStatus}; no new dry run is permitted until the agent is running.`}</p></>
                : <><span>RUNTIME VIEW · SYNTHETIC</span><b>{selectedStatus === "running" ? "Observation path available" : selectedStatus === "paused" ? "Observation path paused" : "Business review required"}</b><p>Status changes affect this concept session only; no agent, credential, source record, or operational schedule is changed.</p></>}
          </div>
        </section>
        ...
        <div className="knowledge-layout">
          ...
          <aside className="entity-inspector">
            {selected ? <>
              ...
              <div className="source-proof"><p className="kicker">SOURCE EVIDENCE · SYNTHETIC REFERENCES</p>{[["SAP S/4HANA","Observed","healthy"],["Teamcenter PLM","Observed","healthy"],["Public filing","Corroborated","info"],["Analyst mapping","Approved","opportunity"]].map((source) => <button type="button" key={source[0]} onClick={() => onToast(`${source[0]} synthetic evidence reference opened; no source system was contacted.`)}><Dot tone={source[2] as StatusTone} /><span><b>{source[0]}</b><small>{source[1]} · source ID retained</small></span><em>→</em></button>)}</div>

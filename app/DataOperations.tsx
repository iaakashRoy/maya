        ...
        <div className="knowledge-layout">
          ...
          <aside className="entity-inspector">
            {selected ? <>
              ...
              <p className="kicker app-use-kicker">USED BY APPLICATIONS</p><div className="entity-apps">{[["Risk Radar","risk"],["Network Optimizer","optimizer"],["Flow Lens","flow"],["Demand Sense","demand"],["Supplier Graph","suppliers"]].map((app) => <button type="button" key={app[0]} onClick={() => onOpenApp(app[1] as "risk" | "optimizer" | "flow" | "demand" | "suppliers")}><span>{app[0].split(" ").map((word) => word[0]).join("").slice(0,2).toUpperCase()}</span><b>{app[0]}</b><em>→</em></button>)}</div>

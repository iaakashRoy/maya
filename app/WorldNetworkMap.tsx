...
export default function WorldNetworkMap({ scope, category, currency, horizon, movement, onMovementChange, onOpenRisk, onOpenOptimizer, onOpenFlow }: WorldNetworkMapProps) {
  ...
  return (
    <section className="network-radar" aria-labelledby="network-radar-title">
      ...
      <div className="radar-kpis" aria-label="Filtered network map metrics">
        <article><span>Tracked asset sample</span><b>{summary.movements}</b><small>{summary.delayed} on watch / critical corridors</small></article>
        <article><span>Tracked cargo sample</span><b>{summary.cargoLots} lots</b><small>{summary.tonnes.toLocaleString()} t · {summary.cubeM3.toLocaleString()} m³</small></article>
        <article><span>Corridor goods value · {currency}</span><b>{formatMoney(summary.goodsValueUsd)}</b><small>{formatMoney(summary.marginExposureUsd)} tracked-sample margin exposed</small></article>
        <article><span>Freight exposure · {currency}</span><b>{formatMoney(summary.freightUsd)}</b><small>{summary.averageEtaVarianceHours >= 0 ? "+" : ""}{summary.averageEtaVarianceHours.toFixed(1)}h mean ETA variance</small></article>
      </div>
      ...
      {viewMode === "Map" ? (
        <div className="radar-map-layout">
          ...
          <aside className="radar-inspector" aria-live="polite">
            ...
            {selectionContext && <div className="radar-inspector-actions"><button type="button" onClick={() => onOpenRisk(selectionContext)}>Trace dependency</button><button type="button" onClick={() => onOpenOptimizer(selectionContext)}>Optimize corridor</button><button type="button" onClick={() => onOpenFlow(selectionContext)}>Inspect cash impact</button><button type="button" onClick={() => setSelection(null)}>Clear selection</button></div>}
          </aside>
        </div>
      ) : (
      ...
      <footer className="radar-timeline">
        ...
        <div className="timeline-change-summary"><article><span>Mean utilization change</span><b>{summary.averageUtilizationDeltaPoints >= 0 ? "+" : ""}{summary.averageUtilizationDeltaPoints.toFixed(1)} pts</b></article><article><span>Freight change</span><b>{formatMoney(view.corridors.reduce((sum, item) => sum + item.costDeltaUsd, 0))}</b></article><article><span>Critical corridors</span><b>{view.corridors.filter((item) => item.status === "critical").length}</b></article><article><span>Evidence events by frame cutoff</span><b>{view.changes.length}</b></article></div>
      </footer>
    </section>
  );
}

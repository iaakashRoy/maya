...
const casePattern: Record<string, string> = { "CASE-1042": "sourcing", "CASE-1041": "transport", "CASE-1038": "sourcing", "CASE-1035": "sourcing", "CASE-1032": "inventory", "CASE-1029": "transport", "CASE-1026": "inventory", "CASE-1024": "network", "CASE-1019": "portfolio" };
...
function ReplayMetric({ label, value, detail, tone = "info" }: { label: string; value: string; detail: string; tone?: StatusTone }) {
  return <article className={`replay-metric replay-metric-${tone}`}><div><span>{label}</span><Dot tone={tone} /></div><b>{value}</b><small>{detail}</small></article>;
}
  ...
  return (
    <div className="formulate-stage">
      <div className="formulation-top">
        ...
        ].map((item) => <article key={item[0]}><span>{item[0]}</span><div><b>{item[1]}</b><small>{item[2]}</small></div><em>{item[3]}</em></article>)}<div className="objective-equation"><span>CONCEPT FORM</span><code>min fixed + purchase + production + transport + inventory + shortage + overtime + duty + carbon + quality + risk</code></div></section>
      </div>
...
function MethodsStage({ pattern, selectedCode, onSelect }: { pattern: (typeof decisionPatterns)[number]; selectedCode: ORMethod["code"]; onSelect: (value: ORMethod["code"]) => void }) {
  ...
  return (
    <div className="methods-stage">
      <section className="recommended-stack"><header className="or-section-head"><div><p className="kicker">RECOMMENDED METHOD STACK</p><h2>{pattern.name}</h2><span>Primary methods solve the decision; supporting methods estimate, stress, explain, or govern it; the fallback protects latency.</span></div><b>{stack.length} selected</b></header><div className="stack-flow">{stack.map((method) => <button type="button" key={method.code} className={`stack-card stack-${methodRole(method, pattern).toLowerCase()}`} onClick={() => onSelect(method.code)}><span>{method.code}</span><div><b>{method.name}</b><small>{methodRole(method, pattern)} · {method.runtime}</small></div><em>Inspect →</em></button>)}</div></section>
...
function StressStage({ result, input }: { result: OptimizationResult; input: OptimizationInput }) {
  ...
  return (
    <div className="app-workspace optimizer-workbench">
      <section className="optimizer-hero">
        <div className="optimizer-hero-code">NO</div><div><p className="kicker">DECISION APPLICATION · {snapshot.shortLabel.toUpperCase()} · {activeCase.id} · SYNTHETIC CONCEPT</p><h1 tabIndex={-1} data-page-heading>Network Optimizer</h1><p>Frame, formulate, calculate, stress, compare, validate, and govern supply-chain decisions across sourcing, production, inventory, logistics, service, cash, carbon, and resilience.</p></div>
        <aside><span>CURRENT DECISION CONTRACT</span><b>{pattern.name}</b><small>{activeCase.title} · {horizon}</small>{networkSelection && <small>Map focus · {networkSelection.kind} · {networkSelection.label}</small>}<button type="button" onClick={runReplay} disabled={running}>{running ? `Calculating fixture · ${progress}%` : pendingChanges.length ? `Recalculate ${pendingChanges.length} pending changes →` : "Recalculate current response →"}</button></aside>
      </section>
      ...
      <div className="or-metrics"><ReplayMetric label="Margin protected" value={`$${result.protectedMargin}M`} detail="Compared with synthetic no-action baseline" tone="opportunity" /><ReplayMetric label="Response cost" value={`$${result.totalCost}M`} detail={`Run authority $${lastRunInput.budgetMillions}M`} /><ReplayMetric label="Projected OTIF" value={`${result.projectedService}%`} detail={`Run hard floor ${lastRunInput.serviceTarget}%`} tone={result.projectedService >= lastRunInput.serviceTarget ? "healthy" : "critical"} /><ReplayMetric label="Residual risk" value={`${result.residualRisk}%`} detail="Post-action synthetic score" tone="watch" /><ReplayMetric label="Carbon delta" value={`${result.carbonDelta > 0 ? "+" : ""}${result.carbonDelta}%`} detail={`Run envelope ${lastRunInput.carbonLimitPercent}%`} tone={result.carbonDelta <= lastRunInput.carbonLimitPercent ? "healthy" : "critical"} /><ReplayMetric label="Review gate" value={pendingChanges.length ? "Stale" : result.releasable ? "Review-ready" : "Blocked"} detail={pendingChanges.length ? `${pendingChanges.length} changes need recalculation` : `${result.hardViolations.length} hard checks failed`} tone={pendingChanges.length ? "watch" : result.releasable ? "opportunity" : "critical"} /></div>
      ...
      {stage === "stress" && <StressStage result={result} input={lastRunInput} />}

...
import { AppGlyph, BrandMark, ClientMark, clientColorFor, IdentityAvatar, NavigationIcon, SectorMark, sectorColorFor } from "./VisualIdentity";
            ...
            <div className="sidebar-path-tree">
              {sidebarPathGroups.map((group) => {
                ...
                return <section key={group.key} className={`sidebar-path-root path-root-${group.kind} ${rootOpen ? "open" : ""}`}>
                  <button data-action-id={`nav.path.root.${group.key}`} className={rootContainsActive ? "active-context" : ""} style={{ "--path-accent": group.kind === "client" ? clientColorFor(group.id) : sectorColorFor(group.id) } as React.CSSProperties} type="button" aria-expanded={rootOpen} aria-controls={rootPanelId} disabled={projectNavQueryActive} onClick={() => { if (rootOpen) { setCollapsedSidebarRoots((current) => current.includes(group.key) ? current : [...current, group.key]); setExpandedSidebarRoots((current) => current.filter((item) => item !== group.key)); } else { setCollapsedSidebarRoots((current) => current.filter((item) => item !== group.key)); setExpandedSidebarRoots((current) => current.includes(group.key) ? current : [...current, group.key]); } }}>{group.kind === "client" ? <ClientMark clientId={group.id} label={group.label} /> : <SectorMark sectorId={group.id} label={group.label} />}<b>{group.label}</b><em>{group.projects.length}</em><i>{rootOpen ? "−" : "+"}</i></button>
                  {rootOpen && <div className="sidebar-path-branches" id={rootPanelId}>
                    {group.branches.map((branch) => {
                      ...
                      return <section key={branch.key} className={`sidebar-path-branch path-branch-${branch.kind} ${branchOpen ? "open" : ""}`}>
                        <button data-action-id={`nav.path.branch.${branch.key}`} className={branchContainsActive ? "active-context" : ""} style={{ "--path-accent": branch.kind === "client" ? clientColorFor(branch.id) : sectorColorFor(branch.id) } as React.CSSProperties} type="button" aria-expanded={branchOpen} aria-controls={branchPanelId} disabled={projectNavQueryActive} onClick={() => { if (branchOpen) { setCollapsedSidebarBranches((current) => current.includes(branch.key) ? current : [...current, branch.key]); setExpandedSidebarBranches((current) => current.filter((item) => item !== branch.key)); } else { setCollapsedSidebarBranches((current) => current.filter((item) => item !== branch.key)); setExpandedSidebarBranches((current) => current.includes(branch.key) ? current : [...current, branch.key]); } }}><span className="path-branch-identity">{branch.kind === "client" ? <ClientMark clientId={branch.id} label={branch.label} /> : <SectorMark sectorId={branch.id} label={branch.label} />}<small>{branch.kind === "client" ? "Client" : "Tower"}</small></span><b>{branch.label}</b><i>{branchOpen ? "−" : "+"}</i></button>
      ...
      <div className="main-shell">
        <div className="shell-chrome">
          ...
          <div className="topbar-actions">
            <button className="search-trigger" type="button" aria-label="Search workspace" title="Search workspace" onClick={() => setSearchOpen(true)}><span aria-hidden="true">⌕</span><b>Search workspace</b><kbd>⌘ K</kbd></button>
            <button data-action-id="nav.variables" className={`variables-trigger ${view === "variables" ? "active" : ""}`} type="button" aria-label="Open Variables and Methods" title="Variables and Methods" aria-pressed={view === "variables"} onClick={() => go("variables")}><NavigationIcon name="variables" /><span className="variables-trigger-label">Variables</span></button>

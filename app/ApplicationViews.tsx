...
function RiskRadar({ project, snapshot, onOpenGraph, onToast }: Pick<ApplicationViewsProps, "project" | "snapshot" | "onOpenGraph" | "onToast">) {
  ...
  const dependencies = projectDependencies ?? [
    { name: "Graphite G-142", family: "Critical material", category: "Critical materials", criticality: 96, probability: 82, exposureM: 4.2, source: "Synthetic graphite processor 01", recovery: "14–22 wk", signal: "Merchant capacity reservation", caseReady: true, tone: "critical" as const },
    ...
    { name: "Program housing", family: "Precision casting", category: "Castings", criticality: 72, probability: 34, exposureM: 2.1, source: "Synthetic casting source 03", recovery: "8–12 wk", signal: "Expansion audit pending", caseReady: true, tone: "opportunity" as const },
      ...
      <div className="app-grid risk-grid">
        ...
        <section className="panel dependency-panel">
          ...
          <div className="dependency-chain"><p className="kicker">EXPOSURE PATH</p>{[
            ["External event", selectedDependency.signal, "Synthetic signal"], ["Supplier", selectedSupplier.name, "Project fixture"], ["Constraint", selectedDependency.name, `${selectedSupplier.dependency}% dependency`], ["Operating node", project ? `${regions[0]} · ${project.name}` : "Program operating node", project ? project.metrics[1].detail : "28 modeled orders"], ["Client outcome", project ? project.outcome : "Priority programs", `$${selectedDependency.exposureM.toFixed(1)}M modeled exposure`],
...
function DemandSense({ project, snapshot, onToast }: Pick<ApplicationViewsProps, "project" | "snapshot" | "onToast">) {
  const primaryFamily = project ? `${project.name} priority portfolio` : "Priority product family";
  ...
  const demandGaps = [
    { family: primaryFamily, row: [`${project?.name ?? "Priority family"} · ${projectRegions[0]}`, "184K", "168K", "−16K", project?.metrics[0].value ?? "$12.8M", project ? project.variablePack.l0[0] : "Graphite G-142"] },

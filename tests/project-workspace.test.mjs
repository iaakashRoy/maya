...
test("ten clients and their cross-tower projects are isolated, evidence-aware, and mounted to distinct app contracts", async () => {
  ...
  assert.equal(projectApps.length, 11);
  assert.equal(new Set(projectApps.map((app) => app.accent)).size, 11);
  assert.equal(new Set(projectApps.map((app) => app.archetype)).size, 11);
...
test("visible project navigation is app-first and keeps data, graph, and identity work connected", async () => {
  ...
  assert.doesNotMatch(workspace, /className="playground-runner-bar"/);
...
test("Playground behaves as a scrollable live agent console with a persistent composer", async () => {
  ...
  assert.doesNotMatch(workspace, /className="playground-app-header"/);
  ...
  assert.match(workspace, /className="agent-message-mark"/);
  assert.match(workspace, /className="agent-prompt-row"/);
  assert.match(workspace, /<textarea rows=\{1\}/);
  assert.match(workspace, /data-tooltip=\{action\.label\}/);
  assert.match(workspace, /label: "Reject source"/);
  assert.match(workspace, /\(\["run", "result", "trace", "apps"\] as const\)/);
  assert.doesNotMatch(workspace, /agent-quick-prompts/);
  const terminalContract = css.slice(css.lastIndexOf("Terminal-first application workspace"));
  assert.match(terminalContract, /\.agent-session\s*\{\s*grid-template-rows:\s*minmax\(0, 1fr\) auto !important;/);
  assert.match(terminalContract, /\.agent-tool-button::after/);
...
test("decision lineage, interactive controls, uniform project leaves, and theme controls are explicit", async () => {
  ...
  assert.match(workspace, /Evidence lineage remains in Data/);
